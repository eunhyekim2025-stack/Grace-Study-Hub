...
// ── In-recording panel: note review (never touches REC) ───────────────────
// Lives INSIDE the .sh-rec container, driven only by the listeners below. It
// never touches REC, the MediaRecorder, or its intervals, and never navigates
// the page (the reader fetches a note's body), so a recording in progress keeps
// running the whole time the panel is open.
type NoteItem = { slug: string; title: string }
type SubjectMeta = { slug: string; emoji: string; label: string; prefixes: string[] }

// The cached note index so reopening the browser is instant.
