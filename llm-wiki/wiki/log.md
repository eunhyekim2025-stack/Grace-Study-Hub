...
## [2026-08-16] notes | Critical Thinking in Real World — textbook chapter notes (11) + private answer key
- Request: from a locally-held PDF of Mooney, Williams & Burik, "An Introduction to Critical and Creative Thinking" (McGraw-Hill, 2015), create chapter-by-chapter CTRW notes; keep the separate answer-key PDF private (my reference for grading Grace's own attempts), never on the site.
- Wrote one concept note per chapter (11), all English, house dc-view style, as original summaries/syntheses (definitions, procedures, worked mini-examples) — NOT reproductions of the textbook: [[critical-thinking-in-real-world/ctrw-ch01-basic-concepts-of-reasoning]] … [[critical-thinking-in-real-world/ctrw-ch11-practical-applications]]. Ch2/4 include mermaid pattern diagrams; Ch5/9 include the standard form/truth-table reference tables; Ch8 the Venn 3-step method. All cross-linked as a chapter chain + to [[case-drug-legalisation-grayling]] (which the textbook itself uses as an example in Ch6).
- Answer key kept OUT of wiki/: durable private copies at ~/.claude/projects/-Users-youngmin-grace-ai/reference/ (outside the git repo); recorded via memory [[ctrw-textbook-answer-key]] + MEMORY.md pointer, structure = Chapter → Exercise number.
- Updated CTRW hub [[critical-thinking-in-real-world/index]] with a "Textbook — chapter notes" table (+ kept the Real-world cases table); index.md home card → "Textbook (11 ch) · Real-world cases 12".

## [2026-08-17] note | CTRW Seminar 1 — recovered from an archived recording after the site save failed
- Trigger: the in-site recorder transcribed the seminar, then `/api/add` failed with GitHub's `Resource not accessible by personal access token` (403) — the fine-grained PAT in Vercel no longer has `Contents: Read and write` on the repo. Last successful site commit was 2026-08-04, ≈30 days after the token was created.
- The browser kept the transcript only in a local variable, so it was lost. The audio was not: `/api/archive` had already put every ~2-min segment in the private Vercel Blob store. Added `api/recordings.js` (list + batched re-transcribe from Blob → Whisper) to read those archives back; recovered 4 segments (~8 min, the seminar's opening) from `recordings/critical-thinking-in-real-world/2026-08-17-ctrw-1`.
- New note [[critical-thinking-in-real-world/seminar-01-what-critical-thinking-is]] (kind 개념): the three active moves, toolkit + attitude, domain neutrality, what critical thinking is not, the three intuition tests (conjunction / Wason cards / base rate) with worked answers, statements vs sentences, facts vs opinions typology, vagueness vs ambiguity, the argument + inference indicators + conditionals, course logistics.
- Built from the 52-slide seminar deck (the recording covers only the opening ~8 min), with the lecturer's spoken framing where the recording had it. Verbatim transcript kept OUT of the public repo at `llm-wiki/raw/ctrw/lectures/2026-08-17-seminar-1-transcript.txt` (gitignored) as the citable source.
- Updated CTRW hub [[critical-thinking-in-real-world/index]] with a new "Seminars" table; index.md home card desc → "Seminars · Textbook (11 ch) · Real-world cases".
- Also hardened the recorder: the stitched transcript is now written to localStorage *before* `/api/add` is called and cleared only on a confirmed commit, with a recovery panel in the modal (retry / copy / discard). A failed save can no longer lose a lecture.

## [2026-08-19] note | ACCT102 Seminar 1 — costs for decision making, from an uploaded class recording
- Request: generate a wiki note from an 18-minute m4a of the closing part of an ACCT102 seminar (uploaded directly, not through the site recorder).
- Transcribed with Groq whisper-large-v3. First pass auto-detected the language as Malay and returned largely garbage for the Singaporean-accented English; re-running with `language=en` plus a domain prompt (cost classification vocabulary) fixed it. Audio converted to 16 kHz mono Opus 48k first — FLAC was lossless and came out at 32 MB, over Groq's 25 MB limit.
